...
static uint16_t mbc_tcp_master_get_route_reg_len(uint8_t command, uint16_t request_reg_len)
{
    switch (command) {
...
static esp_err_t mbc_tcp_master_send_request_internal(void *ctx, mb_param_request_t *request, void *data_ptr, uint32_t timeout_ms)
{
    ...
    if (semaphore_taken == pdTRUE) {
        ...
        uint16_t route_reg_len = mbc_tcp_master_get_route_reg_len(mb_command, mb_size);
